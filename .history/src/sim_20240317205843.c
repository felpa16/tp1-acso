...
inst_t decode(int inst) { // Esto debería devolver un puntero
    ...
    if (decoded.opcode == 0b0101011 || 0b0110001 || 0b1101011 || 0b1110001 || 0b1101010 || 0b1001010) { // Cualquier instrucción que use Rn y Rd con opcode de 7 bits
        decoded.rn = (inst & (r_mask << 5)) >> 5;
        decoded.rd = (inst & r_mask);

        if (decoded.rd == 31) decoded.cmp = 1; else decoded.cmp = 0;

        if (decoded.opcode == 0b0101011 || 0b1101011 || 0b1101010) { // Cualquier instrucción que use Rm
        ...
        else if (decoded.opcode == 0b0101011 || 0b1101011 || 0b1101010 && ext_or_shift == 's') { // Cualquier instrucción que use imm6
