...
inst_t decode(int inst) { // Esto debería devolver un puntero
    ...
    if (decoded.opcode == 0b0101011 || 0b0110001 || 0b1101011 || 0b1110001) { // Cualquier instrucción que use Rn y Rd
        ...
        if (decoded.opcode == 0b1101011 || 0b0101011 && ext_or_shift == 's') { // Cualquier instrucción que use shift y tenga el bit 21 = 0
            decoded.shift = (inst & (two_mask << 22)) >> 22;
        }

        if (decoded.opcode == 0b0110001) { // Cualquier instrucción que use shift independientemente del bit 21
            decoded.shift = (inst & (two_mask << 22)) >> 22;
        }
        ...
        else if (decoded.opcode == 0b0101011 && ext_or_shift == 's') { // Cualquier instrucción que use imm6
            decoded.imm = (inst & (six_mask << 10)) >> 10;
        }

        if (decoded.opcode == 0b0110001) { // Cualquier instrucción que use imm12
