...
inst_t decode(int inst) { // Esto debería devolver un puntero
    ...
    if (decoded.opcode == 0b0101011 || 0b0110001 || 0b1101011) { // Cualquier instrucción que use Rn y Rd
        ...
        if (decoded.opcode == 0b0110001 || 0b0101011) { // Cualquier instrucción que use shift y tenga el bit 21 = 0
            decoded.shift = (inst & (two_mask << 22)) >> 22;
        }

        if (decoded.opcode == 0b1101011) { // Cualquier instrucción que use shift independientemente del bit 21
            decoded.shift = (inst & (two_mask << 22)) >> 22;
        }

        if (decoded.opcode == 0b0101011 || 0b1101011) { // Cualquier instrucción que use imm3
            decoded.imm = (inst & (three_mask << 10)) >> 10;
        }

        else if (decoded.opcode == 0b0101011) { // Cualquier instrucción que use imm6
