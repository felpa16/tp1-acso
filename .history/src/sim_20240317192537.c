...
inst_t decode(int inst) { // Esto debería devolver un puntero
    ...
    if (decoded.opcode == 0b0101011 || 0b0110001 || 0b1101011) { // Cualquier instrucción que use Rn y Rd
        ...
        if (decoded.opcode == 0b0101011 || 0b0101011) { // Cualquier instrucción que use Rm
            decoded.rm = (inst & (r_mask) << 16) >> 16;
        }

        if (inst & (bit_mask << 21) == 1) { // Para diferenciar entre instrucciones extended o shifted register
            ext_or_shift = 'e';
        }
        else ext_or_shift = 's';
